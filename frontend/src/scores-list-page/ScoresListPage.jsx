...
import { Activity , More} from "iconsax-react";
import SearchBox from "./components/SearchBox";
import Button from "../components/Button.jsx";
import { useAuth } from "../auth/context/AuthContext.jsx"; // Importing useAuth hook


// Helper function to format numbers to Persian numerals using toLocaleString
const formatToPersianNumber = (num) => {
  if (num === null || num === undefined || num === '') return '-'; // Handle null, undefined, or empty values
...
export default function GradeReportPage() {
  ...
  const StudentGradeReportView = () => {
    ...
    return (
      <>
        ...
        <div className="flex flex-col lg:flex-row gap-6"> {/* Changed to flex-row */}
          {/* Grade Details Card (Visual Left in RTL) */}
          <div className="lg:w-1/3 bg-white p-7 rounded-xl shadow-xl border border-gray-100 flex-shrink-0 lg:order-first"> {/* Added lg:order-first */}
            <h3 className="text-2xl font-bold mb-6 text-gray-800 text-right">
              جزئیات نمره
            </h3>
            {/* Placeholder for grade details - this would be dynamically filled */}
              ...
              <table className="w-full border-collapse text-center" dir="rtl">
                <thead className="sticky top-0 bg-white z-10">
                  <tr className="border-b border-gray-300 text-gray-400 text-sm">
                    <th className="py-3 px-4 text-center">درس</th> {/* Moved 'درس' to first column */}
                    <th className="py-3 px-4 text-center">نمره نهایی</th>
                    <th className="py-3 px-4 text-center">وضعیت</th> {/* Moved 'وضعیت' to third column */}
                    <th className="py-3 px-4 text-center">جزئیات</th> {/* New column for details */}
                  </tr>
                </thead>
                <tbody>
                  {filteredStudentCourses.length > 0 ? (
                    filteredStudentCourses.map((course) => (
                      ...
                      >
                        <td className="py-3 px-4 text-center"> {/* Cell for 'درس' */}
                          {course.course}
                        </td>
                        <td className="py-3 px-4 font-bold text-blue-700 text-center"> {/* Cell for 'نمره نهایی' */}
                          {formatToPersianNumber(course.finalGrade)}
                        </td>
                        <td className="py-3 px-4 text-center"> {/* Cell for 'وضعیت' */}
                          <span className={`inline-block px-3 py-1 rounded-full text-xs font-semibold ${statusColors[course.status]}`}>
                            {course.status}
                          </span>
                        </td>
                    ...
                    <tr>
                      <td colSpan={4} className="text-center py-8 text-gray-500 text-lg"> {/* colSpan changed to 4 */}
                        نتیجه‌ای یافت نشد
                      </td>
                    </tr>
                  )}
  ...
  // --- End of Student View Component ---


  return (
    <div
      dir="ltr" // Consistent RTL for the whole page
      className="w-full max-w-6xl mx-auto py-10 px-6 sm:px-8 lg:px-10 text-gray-800"
    >
      {/* Header/Title Section */}
      <h2 className="text-4xl font-extrabold mt-6 mb-10 flex items-center border-b border-gray-200 pb-8 gap-3 justify-end --color-big-stone-950">
        <span>گزارش نمرات</span>
        <div className="flex items-center gap-2 self-stretch z-10 relative"> {/* items-center for vertical alignment */}
      ...
      {userRole === "Instructor" ? (
        <>
          ...
          <div className="flex flex-wrap justify-between items-center mb-12 gap-6 relative z-20"> {/* Changed justify-between to justify-end */}
                  ...
                  {filteredGrades.length > 0 ? (
                    filteredGrades.map((student) => (
                      ...
                      >
                        ...
                        <td className="py-3 px-4">{formatToPersianNumber(student.finalTerm)}</td>
                        <td className="py-3 px-4">{formatToPersianNumber(student.midterm)}</td>
                        <td className="py-3 px-4">{formatToPersianNumber(student.phase2)}</td>
                        <td className="py-3 px-4">{formatToPersianNumber(student.phase1)}</td>
                        <td className="py-3 px-4">{formatToPersianNumber(student.quiz3)}</td>
