...
export default function GradeReportPageStudent() {
  ...
  return (
    <div className="container mx-auto bg-white min-h-screen text-gray-800">
      ...
      {/* CRITICAL FIX: Added items-start to prevent vertical stretching */}
      <div className="flex flex-col lg:flex-row gap-6 lg:items-start"> {/* Added lg:items-start here */}
              ...
              <tbody>
                {overallScores.length > 0 ? (
                  overallScores.map((course) => (
                    <tr
                      key={course.classId}
                      className={`border-b border-gray-100 hover:bg-gray-50 transition cursor-pointer ${selectedClassId === course.classId ? "bg-blue-50" : ""
                        }`}
                  ...
                  <tr>
                    ...
                    >
                      ...
                      />
                      <p className="text-gray-600 mb-4 font-bold text-lg">نتیجه‌ای یافت نشد!</p>
                    </td>
                  </tr>
                )}
              </tbody>
              ...
              <div className="flex flex-row justify-between mb-6 pb-4 border-b border-gray-200">
                <div className="flex flex-row-reverse items-center justify-end text-gray-600 text-md">
                  <div className="flex flex-row-reverse items-center justify-end text-gray-600 text-md mb-2">
                    <span className="font-bold text-gray-800" data-testid="selected-class-title">
                      {currentSelectedClassOverallScore.classTitle}
                    </span>
                    <span className="font-semibold ml-1" data-testid="selected-class-title-label"> {/* Added ml-1 to "درس:" for space before value */}
                      درس:
                    </span>
                  </div>
                </div>
                <div className="flex flex-row-reverse items-center justify-end text-gray-600 text-md mb-2">
                  <span className="font-semibold text-gray-800 ml-2" data-testid="selected-class-overall-score">
                    {formatToPersianNumber(currentSelectedClassOverallScore.score)}
                  </span>
                  <span className="font-semibold" data-testid="selected-class-overall-score-label">
                    نمره نهایی:
                  </span>
                </div>
              </div>
