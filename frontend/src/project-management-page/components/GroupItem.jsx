...
const GroupItem = ({ team, userRole, currentUserId, onDeleteTeamRequest }) => {
    ...
    return (
        <div
            ...
            data-testid={`group-item-${team.id}`}
        >
            ...
            <div
                ...
                data-testid={`group-item-header-${team.id}`} 
            >
                <span className="flex-grow text-base font-bold flex items-center" data-testid={`group-item-name-${team.id}`}>
                    {team.name}
                </span>
                {userRole === "Instructor" && ( 
                    <div className="relative"> {/* والد برای Tooltip */}
                        <Trash
                            size="20"
                            variant="Outline"
                            color="#a0aec0"
                            ...
                            data-testid={`delete-team-icon-${team.id}`} 
                        />
                        {showTooltip && (
                            <div className="absolute top-1/2 left-full -translate-y-1/2 transform bg-gray-700 text-white text-xs rounded py-1 px-2 whitespace-nowrap z-50 mr-2"
                                data-testid={`delete-team-tooltip-${team.id}`}> 
                                حذف تیم
                            </div>
                        )}
                    </div>
                ...
                <ArrowDown2
                    ...
                    `}
                    data-testid={`group-item-toggle-icon-${team.id}`} 
                />
            </div>

            {/* بخش نمایش اعضا */}
            {isOpen && (
                <div className="members-section mt-4 pt-4 border-t border-gray-700" data-testid={`group-members-list-${team.id}`}>
                    {loadingMembers && (
                        <div className="flex items-center justify-center text-xs text-gray-400" data-testid={`loading-members-message-${team.id}`}>
                    ...
                    {membersError && (
                        <div className="flex items-center justify-center text-xs text-red-400" data-testid={`members-error-message-${team.id}`}>
                            <InfoCircle size="16" className="ml-1" />
                            {membersError}
                        </div>
                    )}
                    {!loadingMembers && !membersError && members.length === 0 && (
                        <div className="flex items-center justify-center text-xs text-gray-400" data-testid={`no-members-found-message-${team.id}`}>
                            <InfoCircle size="16" className="ml-1" />
                            عضوی یافت نشد.
                        </div>
                    )}
                        ...
                        <ul className="text-xs font-medium text-gray-200">
                            {members.map(member => (
                                <li
                                    ...
                                    `}
                                    data-testid={`group-member-item-${member.studentId}`} 
                                >
                                    ...
                                    <span className="text-right flex-grow" data-testid={`group-member-name-${member.studentId}`}>
                                        {member.displayName}
                                    </span>
                                </li>
                            ))}
