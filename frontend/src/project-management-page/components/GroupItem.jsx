...
import { ArrowDown2, User, InfoCircle } from "iconsax-react";

const GroupItem = ({ team, userRole, currentUserId, fetchTeamMembers }) => {
  ...
  const handleToggle = async () => {
    if (!isOpen && members.length === 0) { 
      ...
      try {
        const fetchedMembers = await fetchTeamMembers(team.id, userRole);
        setMembers(fetchedMembers);
      } catch (err) {
        console.error("Error fetching team members:", err);
        setMembersError("خطا در بارگذاری اعضا.");
      } finally {
      ...
      >
        ...
        <ArrowDown2
          ...
          className={`
            ...
            ${isOpen ? 'rotate-180' : ''}
            group-hover:text-blue-400 // رنگ فلش در هاور (استفاده از group-hover)
          `}
        />
      </div>
              ...
              {members.map(member => (
                ...
                >
                  ...
                  <span className="text-right flex-grow">
                    {userRole === "Instructor" ?
                      `${member.fullName && member.fullName.trim() !== '' ? member.fullName.trim() : ''}${member.studentNumber ? ` (${member.studentNumber})` : ''}`.trim()
                      :
